...
def logError(username, data):
    x = {'username': username, 'datetime': timepicker.now().strftime(
        "%d/%m/%Y %H:%M:%S"), 'detail': data}
    path = './config/errorlogs.txt'

    checkAndCreateJson(path)

    pr("ERROR: " + str(data['message']), 'X')
...
def setUsernames():
    ...
    while True:
        username = input(
            "[?] Kullanıcı adı girin (Başlatmak için boş bırak): ")
        if username.strip() == "":
            break
...
def getStoryTime(driver):
    datetime = get_attribute_xpath(
        driver, '//*[@id="react-root"]/section/div[1]/div/section/div/header/div[2]/div[1]/div/div/div/time', 'datetime')
    date = get_attribute_xpath(
        driver, '//*[@id="react-root"]/section/div[1]/div/section/div/header/div[2]/div[1]/div/div/div/time', 'title')
    day = date.split(' ')[0]
...
def startStories(driver):
    click_to_element_xpath(
        driver, '//*[@id="react-root"]/section/div[1]/div/section/div/div[1]/div/div/button')


def stopResumeStory(driver):
    click_to_element_xpath(
        driver, '//*[@id="react-root"]/section/div[1]/div/section/div/header/div[2]/div[2]/button[1]')


def nextStory(driver):
    if get_by_element_xpath(driver, '//*[@id="react-root"]/section/div[1]/div/section/div/button[2]/div'):
        click_to_element_xpath(
            driver, '//*[@id="react-root"]/section/div[1]/div/section/div/button[2]/div')


def recognizeVideo(driver):
    try:
        get_by_element_xpath(
            driver, '//*[@id="react-root"]/section/div[1]/div/section/div/div[1]/div/div/video')
    except:
        return False
    return True
...
def getVideo(driver, username):
    try:
        url_link = get_attribute_xpath(
            driver, '//*[@id="react-root"]/section/div[1]/div/section/div/div[1]/div/div/video/source', 'src')
        datetime = getStoryTime(driver)
        ...
        logError(username, {'url': driver.current_url, 'exception': str(
            e), 'message': 'video did not download'})
        pr('Bu hikaye indirilemedi.', '!!!')


def getImage(driver, username):
    try:
        pic_url = get_attribute_xpath(
            driver, '//*[@id="react-root"]/section/div[1]/div/section/div/div[1]/div/div/img', 'src')
        ...
        logError(username, {'url': driver.current_url, 'exception': str(
            e), 'message': 'image did not download'})
        pr('Bu hikaye indirilemedi.', '!!!')
...
def checkStory(driver):
    try:
        get_by_element_xpath(
            driver, '//*[@id="react-root"]/section/div[1]/div/section/div/div[1]')
    except:
        return False
    return True
...
def recordScan(username, sType):
    ...
    with open(path, mode='r', encoding='utf-8') as feedsjson:
        ...
        for data in feeds:
            ...
            if data["username"] == username:
                ...
                if sType in data:
                    scanCount = data[sType]

                feeds[index][str(sType)] = scanCount + 1
    ...
    if not found:
        with open(path, mode='w', encoding='utf-8') as feedsjson:
            feeds.append({'username': username, sType: 1, 'scans': [
                         {'type': sType, 'datetime': datetime}]})
            json.dump(feeds, feedsjson)
...
def recordProfile(driver, username):
    ...
    try:
        fullname = get_by_element_xpath(
            driver, '//*[@id="react-root"]/section/main/div/header/section/div[2]/h1').text
    except Exception as e:
        logError(username, {'url': driver.current_url, 'exception': str(
            e), 'message': 'profile fullname did not download'})

    try:
        desc = get_attribute_xpath(
            driver, '//*[@id="react-root"]/section/main/div/header/section/div[2]/span', 'innerHTML')
    except Exception as e:
        logError(username, {'url': driver.current_url, 'exception': str(
            e), 'message': 'profile desc did not download'})

    try:
        link = get_by_element_xpath(
            driver, '//*[@id="react-root"]/section/main/div/header/section/div[2]/a').text
    except Exception as e:
        logError(username, {'url': driver.current_url, 'exception': str(
            e), 'message': 'profile link did not download'})

    try:
        follower = get_attribute_xpath(
            driver, '//*[@id="react-root"]/section/main/div/header/section/ul/li[2]/a/span', 'title')
    except Exception as e:
        logError(username, {'url': driver.current_url, 'exception': str(
            e), 'message': 'profile follower did not download'})

    try:
        following = get_by_element_xpath(
            driver, '//*[@id="react-root"]/section/main/div/header/section/ul/li[3]/a/span').text
    except Exception as e:
        logError(username, {'url': driver.current_url, 'exception': str(
            e), 'message': 'profile following did not download'})

    try:
        postCount = get_by_element_xpath(
            driver, '//*[@id="react-root"]/section/main/div/header/section/ul/li[1]/span/span').text
    except Exception as e:
        logError(username, {'url': driver.current_url, 'exception': str(
            e), 'message': 'profile post count did not download'})

    datetime = timepicker.now().strftime("%d/%m/%Y %H:%M:%S")

    checkAndCreateFile('./instagram/'+username)

    x = {'username': username, 'datetime': datetime, 'detail': {'fullname': fullname,
                                                                'postCount': postCount, 'follower': follower, 'following': following, 'desc': desc, 'link': link}}
