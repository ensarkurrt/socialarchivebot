...
def logError(username,data):
    ...
    with open('errorlogs.txt', mode='r', encoding='utf-8') as feedsjson:
        feeds = json.load(feedsjson)
    with open('errorlogs.txt', mode='w', encoding='utf-8') as feedsjson:
        entry = x
        feeds.append(entry)
...
def recordProfile(driver,username):
    ...
    following = get_attribute_xpath(driver,'//*[@id="react-root"]/section/main/div/header/section/ul/li[3]/a/span','title')
    ...
    if not os.path.exists('./instagram/'+username+'/profile.txt'): 
        with open('./instagram/'+username+'/profile.txt', mode='w', encoding='utf-8') as file:
            file.write('[]')
    with open('./instagram/'+username+'/profile.txt', mode='r', encoding='utf-8') as feedsjson:
        feeds = json.load(feedsjson)
    with open('./instagram/'+username+'/profile.txt', mode='w', encoding='utf-8') as feedsjson:
        entry = x
        feeds.append(entry)
        json.dump(feeds, feedsjson)
