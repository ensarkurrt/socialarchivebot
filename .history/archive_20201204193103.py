    ...
    with open(path, mode='r', encoding='utf-8') as feedsjson:
        ...
        for data in feeds:
            ...
            if data["username"] == username:
                ...
                count = data.appen({'count'})
                if str(sType) in count:
                    scanCount = count[str(sType)]

                feeds[index]['count'][str(sType)] = scanCount + 1
        ...
        if not found:
            with open(path, mode='w', encoding='utf-8') as feedsjson:
                    feeds.append({'username':username,'profile':0,'story':0,'scans': [{'type':sType,'datetime':datetime}]})
                    json.dump(feeds, feedsjson)
