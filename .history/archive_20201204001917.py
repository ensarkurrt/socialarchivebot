...
def createFileSystem(datetime,username,filetype):
    return 'stories/'+username+'/'+datetime['year']+'/'+datetime['mounth']+'/'+datetime['day']+'/'+datetime['hour']+'_'+datetime['minute']+'_'+datetime['second']+'_'+type
...
def getVideo(driver,username):
    ...
    urllib.request.urlretrieve(url_link, 'stories/'+username+'/'+datetime['year']+'/'+datetime['mounth']+'/'+datetime['day']+'/video_'+username+'.mp4')
