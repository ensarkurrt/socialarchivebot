...
def find_name(l, value):
     return l[index(l, lambda item: item["username"] == value)]
...
def recordScan(username,sType):
    ...
    with open(path, mode='r', encoding='utf-8') as feedsjson:
        ...
        try:
            data = find_name(feeds,username)
            #data = print(feeds.index('reynmen'))
        except Exception as e:
            print('kayıt bulunamadı') 
            with open(path, mode='w', encoding='utf-8') as feedsjson:
                feeds.append({'username':username})
                json.dump(feeds, feedsjson)
    ...
    #x = {'username':username,'type':sType,'datetime':datetime}
