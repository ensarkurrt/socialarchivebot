...
def getStoryTime(driver):
    ...
    return {'day': datetime[8,10],'mounth': datetime[5,7],'year': datetime[0,4],'hour': datetime[11,13],'minute':datetime[14,16],'second:'time[17,19]}
    return [time[11,13],time[14,16],time[17,19]]
...
def getVideo(driver,username):
    ...
    time = getStoryTime(driver)
    urllib.request.urlretrieve(url_link, 'stories/'+username+'/'+time[]+'/video_'+username+'.mp4')
